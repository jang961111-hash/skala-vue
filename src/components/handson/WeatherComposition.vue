<!--
  ...
  요구사항 대조표
  ────────────────────────────────────────────────────────────────
  [v] 1. 반응형 상태 관리 — searchQuery / selectedCityInfo / weatherList  → 121p ref()
  [v] 2. computed 로 filteredWeatherList                                → 127p computed()
  [v] 3. watch 로 selectedCityInfo 감시 / watchEffect 로 searchQuery 추적 → 130p, 141p
  [v] 4. 검색 결과 3분기 (빈 검색어 / 일치 / 불일치)                       → template
  [v] 5. 본인만의 상태·computed·watcher 추가                              → 아래 [요구사항 5]

  ▸ 116p 버전(WeatherMockup.vue)과의 차이
    filteredWeatherList  일반 함수  → filteredWeatherList  computed
    averageTemp   일반 함수  → averageTemp          computed
    (없음)                        → watch / watchEffect 로 변화 감시
  ================================================================
-->
<script setup>
...
/* ================================================================
   [요구사항 2] computed 로 검색 결과 필터링   (교재 127p)
   ================================================================
   ▸ 116p 버전에서는 filteredWeatherList 라는 일반 함수였다.
     일반 함수는 화면이 다시 그려질 때마다 매번 재실행된다.
     computed 는 의존하는 반응형 데이터(searchQuery / onlyRainy / weatherList)가
     바뀔 때만 재계산하고, 그 외에는 캐싱된 값을 그대로 돌려준다. (127p)

   ▸ Syntax:  const 식별자 = computed(() => { return 값 })
     인자로 콜백 함수를 넣고, 콜백이 반환한 값이 Computed Ref 객체에 담긴다.
     <script setup> 안에서 읽을 때는 .value 를 붙여야 한다.
     (template 안에서는 Vue 가 자동으로 벗겨주므로 그냥 쓴다)

   ▸ computed 는 읽기 전용이다. filteredWeatherList.value = ... 로 재할당할 수 없다.
   ================================================================ */
...
/* [요구사항 5] 개인 추가 computed — 표시 중인 도시들의 평균 기온
   filteredWeatherList 를 의존하므로, 검색·필터가 바뀔 때만 재계산된다.
   computed 가 다른 computed 를 참조하는 형태다. */
...
/* [요구사항 5] 개인 추가 computed — 검색 상태를 3가지로 분류
   요구사항 4의 "빈 검색어 / 일치 / 불일치" 분기를 template 에서 읽기 쉽게
   문자열 하나로 미리 계산해 둔다. template 의 v-if 조건이 단순해진다. */
...
/* ================================================================
   [요구사항 3] 반응형 변수 변화 감시   (교재 130p watch / 141p watchEffect)
   ================================================================
   ▸ watch 와 watchEffect 의 차이 — 이 과제가 둘 다 시키는 이유

     watch(감시대상, (newVal, oldVal) => {...})
       · 감시할 대상을 "명시"한다
       · 이전 값(oldVal)을 받을 수 있다
       · 최초에는 실행되지 않는다 (값이 바뀌어야 실행)

     watchEffect(() => {...})
       · 콜백 안에서 "접근한" 반응형 데이터를 자동으로 추적한다 (명시 불필요)
       · 이전 값을 주지 않는다 (현재 시점만)
       · 컴포넌트가 생성될 때 최초 1회 즉시 실행된다

   ▸ 그래서 용도가 갈린다
     "무엇이 무엇으로 바뀌었나"를 알아야 하면 watch,
     "지금 값이 이렇다"만 필요하면 watchEffect.
   ================================================================ */

// [요구사항 3-①] selectedCityInfo 감시 — 상태바 문구가 바뀔 때마다 콘솔 로그
//   이전 값이 필요하므로 watch 를 쓴다. (어느 도시에서 어느 도시로 옮겨갔는지)
watch(selectedCityInfo, (newVal, oldVal) => {
  console.log(`[watch] 선택 도시: "${oldVal || '(없음)'}" → "${newVal}"`)
})

// [요구사항 3-②] searchQuery 추적 — 타이핑할 때마다 콘솔 로그
//   현재 검색어와 그 결과만 알면 되므로 watchEffect 가 맞다.
//   콜백 안에서 searchQuery 와 filteredWeatherList 에 접근하므로 둘 다 자동 추적된다.
//   ※ 컴포넌트 마운트 시 1회 즉시 실행되므로, 초기 상태도 로그에 남는다.
watchEffect(() => {
  console.log(
...
// [요구사항 5] 개인 추가 watcher — 표시 중인 도시가 0건이 되는 순간을 잡는다
//   averageTemp 가 아니라 '건수'를 감시해, 검색 결과가 사라진 시점만 알린다.
watch(
  ...
  <div class="weather-dashboard">
    ...
    <section class="stat-row">
      <div class="stat-chip">
        ...
        <span class="stat-value">{{ filteredWeatherList.length }}<small>곳</small></span>
      </div>
      <div class="stat-chip">
        <span class="stat-label">평균 기온</span>
        <span class="stat-value">{{ averageTemp }}<small>℃</small></span>
      </div>
    </section>
    ...
    <!-- ============================================================
         [요구사항 4] 검색 결과 표시 — 3가지 상태로 분기
         ------------------------------------------------------------
         searchState computed 가 'empty' / 'found' / 'notfound' 중 하나를
         돌려주므로, template 의 조건이 단순해진다.
         · empty    : 검색어가 비었음 → 원본 전체를 그대로 보여주는 중
         · found    : 일치하는 도시가 있음 → 몇 건인지 안내
         · notfound : 일치하는 도시가 없음 → 안내 문구
         ============================================================ -->
    <p v-if="searchState === 'notfound'" class="empty-state">
      🔍 "{{ searchQuery }}" 와 일치하는 도시가 없습니다. 검색어나 필터를 확인해 주세요.
    </p>
    <p v-else-if="searchState === 'found'" class="search-hint">
      🔎 "{{ searchQuery }}" 검색 결과 {{ filteredWeatherList.length }}건
    </p>
    <p v-else class="search-hint muted">전체 {{ weatherList.length }}개 도시를 표시 중입니다.</p>
  </div>
</template>
